...
**D13 — Observabilidade.** O audit trail canônico permanece exclusivamente no control plane do Pipe (SQLite local, hash-chained). Qualquer desenho futuro de integração DSH só pode persistir metadados redigidos: identificadores, sequência, hashes de versão/protocolo/perfil/workspace, contagens agregadas e estados de execução. Prompt bruto, saída de modelo, argumento de ferramenta, frame de protocolo bruto ou log de sessão completo do DSH nunca são persistidos no control plane nem no repositório; se necessários para depuração de um spike futuro, ficam em diretório temporário local, efêmero e descartado ao final, nunca versionado. *(Aprovação humana registrada em 2026-09-10.)*
...
| `execute` | Rodar apenas um ticket aprovado ou pedido de planejamento. | Executaria, no máximo, um workflow `review`/`check` (D2) sob um `ApprovalRecord` Pipe explícito; qualquer ferramenta com efeito de escrita fica fora de escopo até revisão própria. | Não executado — apenas desenhado |
...
| `handoff` | Escrever ou propor handoff canônico para Linear/GitHub/repositório. | Proporia apenas documentos inertes (`pipe_propose`-like), nunca aplicaria mutação; aplicação real permanece atrás do `ApplyService`/adapter governado existente. | Não executado — apenas desenhado |
...
- um ticket de spike técnico (Fase 1) rodar contra fixtures reais e revelar que alguma decisão D1–D13 é inexequível como desenhada;
...
| 4 | Um workspace fora da allowlist, contendo symlink não resolvido, ou apontando para o worktree real do Pipe | é preparado para uma sessão | a preparação é recusada antes de qualquer lançamento de runtime (D7) |
