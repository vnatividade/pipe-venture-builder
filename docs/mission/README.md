...
- `commit_if_needed` e `push_branch` (o commit e o push do supervisor) sempre rodam com `-c core.hooksPath=<diretório vazio, fora de qualquer worktree, criado pelo próprio supervisor>`. `core.hooksPath` é relativo à árvore de onde o `git` é chamado; sem essa flag, um `git commit`/`git push` do supervisor com `cwd` no worktree da missão executaria `pre-commit`/`commit-msg`/`post-commit`/`pre-push` de dentro daquele worktree — código que o worker escreveu — com as credenciais do supervisor. Foi assim que a PIP-904 v1 alterou o repositório real (`core.bare=true`, branch movida) em 11/09. `--no-verify` não bastaria: ele pula `pre-commit`/`commit-msg`/`pre-push`, mas não `post-commit`. A verificação real continua sendo o diff/critérios/revisor da missão e, depois do PR, a CI (`requireChecks`) — a entrega não promete mais garantia do que isso.
- `child_env()` (em `delivery.py`, reaproveitado por `verify.py`) remove, além das variáveis do interpretador Python, `GIT_DIR`, `GIT_WORK_TREE`, `GIT_INDEX_FILE`, `GIT_OBJECT_DIRECTORY`, `GIT_COMMON_DIR`, `GIT_ALTERNATE_OBJECT_DIRECTORIES`, `GIT_PREFIX`, `GIT_QUARANTINE_PATH` e `GIT_NAMESPACE` do ambiente de **todo** filho `git`/`gh` do supervisor (worktree, snapshot, diff, commit, push, PR) e de **todo** comando `check` de critério (`verify.run_check`). Um hook de worktree vinculado exporta essas variáveis (`GIT_DIR` absoluto incluído) para o processo que ele lança; se um filho do supervisor herdasse isso, operaria no repositório errado.
