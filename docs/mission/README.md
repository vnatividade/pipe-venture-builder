...
- **Quando roda**: só quando o worker termina um ciclo com `done: false` e `blockers` não vazio, a missão declara `delegation.answerBlockers`, o texto dos `blockers` não bate a guarda de termos sensíveis (abaixo), e o número de respostas delegadas já dadas (`decision.delegated` com `rule: answerBlockers`) está abaixo de `maxTimes`. Fora isso — sem a regra, guarda disparada, ou limite esgotado — o respondedor nunca é chamado e a missão escala exatamente como antes do PIP-906.
- **O que é**: um `claude -p` **somente leitura**, no mesmo worktree do ciclo, com o mesmo isolamento do worker e do revisor (`--setting-sources project --strict-mcp-config` + o deny-list de `isolation_args()`) e `--allowedTools "Read,Grep,Glob"` — **sem Edit, Write nem Bash**: ele não pode alterar nada nem executar comando algum, só ler arquivos para ver se a resposta já está aí. `--permission-mode plan`, `--json-schema` com `{"action": "instruct"|"escalate", "instructions": str, "reason": str}`. O prompt traz a missão (intenção, critérios, não-objetivos, reservado ao fundador, write set) e os `blockers` do worker — nunca a transcrição do worker.
...
- **GUARDA DETERMINÍSTICA** (`supervisor.contains_sensitive_terms`, não depende do modelo): se o texto dos `blockers` **ou** das `instructions` do respondedor menciona credencial/segredo (`credencial`, `segredo`, `secret`, `api key`, `token`, `senha`, `password`), merge, produção/deploy, ou cobrança/billing/pagamento, a missão escala como hoje (`paused` + `clarification` pendente) — mesmo que o respondedor diga `instruct`. A guarda nos `blockers` evita até chamar o respondedor; a guarda nas `instructions` cobre o caso do próprio respondedor sugerir algo sensível.
- **Escala como hoje** (decisão `clarification` pendente, respondedor nunca chamado de novo neste ciclo): `action: "escalate"`, `instructions` vazio ou só espaço, o run do respondedor falhar (`responder_run_failed`) ou sair sem uma resposta válida no schema (`responder_output_invalid`), `maxTimes` esgotado, ou a missão não ter `delegation.answerBlockers`.
- **Nunca delegável**: nada em `reservedToHuman`, os gates absolutos (`productionAllowed` etc.), credenciais, merge, produção, billing ou comunicação externa — a guarda determinística é o piso; o prompt do respondedor também instrui `escalate` para qualquer coisa fora do escopo da missão.
- Testes: `tests/mission/test_responder.py` (prompt, comando, isolamento, parsing), `tests/mission/test_decisions_evidence.py::AnswerBlockersDelegatedDecisionTests` (store), `tests/mission/test_supervisor.py::AnswerBlockersDelegationTests`/`SensitiveTermsGuardTests` (ciclo completo e a guarda), além do contrato em `test_contract.py`.
...
O revisor recebe as mesmas flags de isolamento e o mesmo deny-list, com `--permission-mode plan` e allow só de leitura (`Read,Grep,Glob,Bash(git diff *),Bash(git log *)`). O respondedor (PIP-906) recebe as mesmas flags de isolamento e o mesmo deny-list também, mas com allow ainda mais estreito — `Read,Grep,Glob`, sem nenhum `Bash` — porque ele só decide se a resposta já está no repositório, nunca inspeciona o diff.
...
"O respondedor de bloqueios" (PIP-906) tem teste próprio: `tests/mission/test_responder.py` (prompt, comando somente-leitura sem Edit/Write/Bash, isolamento, parsing da resposta), `tests/mission/test_contract.py` (limites de `answerBlockers` e a combinação com `grantCycle`), `tests/mission/test_decisions_evidence.py::AnswerBlockersDelegatedDecisionTests` (o store só resolve `clarification`/`retry` com `context.reason == "worker_blockers"`, dentro de `maxTimes`, e nunca confunde a regra com `grantCycle`) e `tests/mission/test_supervisor.py::AnswerBlockersDelegationTests`/`SensitiveTermsGuardTests` (o ciclo completo com o `fake_claude.py` distinguindo respondedor de revisor pelo `--json-schema`, e a guarda determinística isolada).
